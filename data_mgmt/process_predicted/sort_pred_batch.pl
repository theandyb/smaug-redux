...
# foreach my $categ (@categs){
  ...
  my $builddatbatch = "$analysisdir/slurm/sort_pred_batch.txt";
  ...
  print $mdFH "srun cat $analysisdir/output/predicted/\$CAT/chr\$INDEX/* | sort -V -k2 > $analysisdir/output/predicted/chr\$INDEX.\$CAT.txt";
